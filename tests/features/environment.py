...
def get_from_config(what):
    if 'Linux' in platform.system():
        ...
        print(os.getcwd())
        with open(os.getcwd() + '/tests/features/config.json') as json_file:
            data = json.load(json_file)
            return data[what]
...
def set_windows_driver(context):
    ...
    options.add_argument('--window-size=1280,1000')
    #options.add_argument('--headless')
    print("My Path is = "+os.path.dirname(os.getcwd()))
    context.browser = webdriver.Chrome(os.getcwd() + '/driver/chromedriver.exe', chrome_options=options
    )
